    ...
    try {
        ...
        switch (role) {
            case 'legal_entity':
                ...
                const company_detail_obj = { company_name: company_detail.company_name, company_incorporation_date: company_detail.company_incorporation_date, company_Id_number: company_detail.company_Id_number, address: company_detail.address }
                const { company_Representative } = company_detail;
                const company_Representative_detail = { name: company_Representative.name, email: company_Representative.email, mobile_number: company_Representative?.mobile_number }
                const { identity_proof, address_proof } = company_Representative
                appUser = await db.appUser.create({ role: req.body.role, created_by_id: req.user?.id }, { transaction });
            ...
            default:
                ...
                const user_detail_obj = { name: user.name, email: user.email, mobile_number: user.mobile_number, address: user.address, profile_image: user.profile_image, access_group: user.access_group };
